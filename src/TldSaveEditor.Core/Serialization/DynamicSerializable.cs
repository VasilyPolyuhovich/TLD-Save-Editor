...
namespace The_Long_Dark_Save_Editor_2.Serialization
{
    public class DynamicSerializable<T> where T : new()
    {
        ...
        private object ParseObject(JObject obj, Type t)
        {
            ...
            foreach (var child in obj)
            {
                if (props.ContainsKey(child.Key))
                {
                    ...
                    try { prop.SetValue(result, childVal); }
                    catch (Exception ex) { throw DescribeAssignError(t, child.Key, childType, childVal, ex); }
                }
                else if (fields.ContainsKey(child.Key))
                {
                    ...
                    var childVal = Parse(child.Value, childType, attr);
                    try { field.SetValue(result, childVal); }
                    catch (Exception ex) { throw DescribeAssignError(t, child.Key, childType, childVal, ex); }
                }
                else
                {
        ...
        private static Exception DescribeAssignError(Type owner, string member, Type memberType, object value, Exception inner)
        {
            var valDesc = value == null ? "null" : $"{value.GetType().Name} '{value}'";
            return new Exception(
                $"Failed to set {owner.Name}.{member} (declared {memberType.Name}) from {valDesc}: {inner.Message}", inner);
        }
